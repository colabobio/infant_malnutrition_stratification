...
# Remove the temporary variable maternal_bmi_value
df$maternal_bmi_value <- NULL
```
...
```{r}
# Assuming your df is named 'df'
# Remove the variable paternal_height if it exists
if ("paternal_height" %in% colnames(df)) {
  df$paternal_height <- NULL
}

# Create a new variable paternal_height_cm and recode it into paternal_height
df <- df %>%
  mutate(paternal_height_cm = hb3 / 10) %>%
  replace_na(list(paternal_height_cm = NA)) %>%
...
# Remove the temporary variable paternal_height_cm
df$paternal_height_cm <- NULL
...
df <- df %>%
  ...
  mutate(firstbirth_18y = case_when(
...
df  <- df %>%
  ...
  replace_na(list(dd_quintile = 0)) # Replace missing values in 'dd_quintile' with 0
```
...
df <- df %>%
  mutate(bf_start = case_when(v426 >= 0 & v426 <= 101 ~ "1. within first hour",
...
cols_to_remove <- grep("^water_source", colnames(df), value = TRUE)
...
df <- df %>% 
  mutate(toilet_type_char = case_when(
    between(hv205, 11, 15) ~ "1. flush toilet",
    between(hv205, 21, 23) | between(hv205, 41, 96) ~ "2. pit or dry toilet",
    hv205 == 31 ~ "0. no toilet",
    TRUE ~ as.character(hv205)
  )) %>% mutate(toilet_type = case_when(
    toilet_type_char == "1. flush toilet"  ~ 1,
     toilet_type_char == "2. pit or dry toilet" ~ 2,
    toilet_type_char == "0. no toilet" ~ 0,
    TRUE ~ as.numeric(hv205)
  )) 
...
df <- df %>% 
  mutate(toiletshare = case_when(
    hv225 == 0 ~ "0. no",
    hv225 == 1 ~ "1. yes",
    TRUE ~ as.character(hv225)
  ))
```
...
df <- df %>%
  ...
  mutate(air_quality = ifelse(fuel_type == 1, 1, air_quality)) %>%
  mutate(air_quality = ifelse(hv242 == 1 & (fuel_type == 2 | fuel_type == 3), 2, air_quality)) %>%
  mutate(air_quality = ifelse(hv242 != 1 & (fuel_type == 2 | fuel_type == 3), 3, air_quality)) %>%
  mutate(air_quality = ifelse((hv241 == 2 | hv241 == 3) & (fuel_type == 2 | fuel_type == 3), 2, air_quality)) %>%
...
# Recode 'sh36' variable into 'caste' variable
#df <- df %>% mutate(caste = case_when(
#  sh36 == 1 ~ "4. Scheduled Caste",
#  sh36 == 2 ~ "3. Scheduled Tribe",
#  sh36 == 3 ~ "2. Other backward caste",
#  sh36 == 4 ~ "1. None",
#  sh36 == 8 ~ NA_character_, # missing
#  TRUE ~ NA_character_ # all other cases
#))

# Replace 'caste' value with 1 if 'sh35' equals 993 or 998
#df <- df %>% mutate(caste = if_else(sh35 %in% c(993, 998), "1. None", caste))
```
...
```{r}
## Unknown variable: s564
# Rename variable 's564' to 'anganwadi_support'
#df <- df %>% rename(anganwadi_support = s564)
```
...
df$covariate_nonmissing <- ifelse(
  ...
  !is.na(df$air_quality) & #!is.na(df$iodized_salt) & # Ionized salt variable not found
...
df$cooking_fuel <- case_when(
  df$fuel_type == 1 ~ 1,
  df$fuel_type == 2 | df$fuel_type == 3 ~ 0,
  TRUE ~ NA_real_
)
...
```{r}
#Unknown Variable: low_caste (due to caste variable not present)
...
#df$low_caste <- ifelse(df$caste == 1, 0, 
#                         ifelse(df$caste > 1 & df$caste < 6, 1, df$low_caste))
```
...
df$toilet[df$toilet_type == 0] <- 0
df$toilet[df$toilet_type == 1 | df$toilet_type == 2] <- 1
```
...
df <- df %>% mutate(iron_3mo = case_when(
  preg_iron %in% c(0, 1, 2) ~ "0. no",
  preg_iron %in% c(3, 4) ~ "1. yes",
  TRUE ~ NA_character_ # all other cases
))
```
...
# Add labels to m_ht
labels <- c("1 5feet2 or higher", "4feet8 to 5feet2", "less than 4feet8")
levels <- c(1, 2, 3)
...
order wgt real_id mother_id father_id bidx haz waz whz stunted wasted underweight ciaf  ///
child_age age_group age_group_5y female ///
maternal_height maternal_bmi marital_age firstbirth_18y preg_deworm preg_ironyn preg_iron delivery_place preg_deworm ///
wealth_quintile hhadults rural state bpl_card caste religion ///
total_living_children siblings sibling_under5 brothers_home sisters_home *birth_interval multiplet ///
dietary_diversity_score bf_start vitamin_a child_anemia child_fever child_diarrhea birthweight child_deworm ///
water_source water_treatment stool_disposal toilet_type toiletshare air_quality anganwadi_support season

```{r}
# Unknown variable: bpl card, caste, birth interval,  child diarrhea, stool_disposal_, anganwadi_support

# Reorder variables in the data frame
df <- df %>% select(wgt, real_id, mother_id, father_id, bidx, haz, waz, whz, stunted, wasted, underweight, ciaf,
                    child_age, age_group, age_group_5y, female, maternal_height, maternal_bmi, marital_age, 
                    firstbirth_18y, preg_deworm, preg_ironyn, preg_iron, delivery_place, preg_deworm,
                    wealth_quintile, hhadults, rural, state, bpl_card, caste, religion,
                    total_living_children, siblings, sibling_under5, brothers_home, sisters_home,
                    birth_interval, multiplet, dietary_diversity_score, bf_start, vitamin_a,
                    child_anemia, child_fever, child_diarrhea, birthweight, child_deworm, water_source,
                    water_treatment, stool_disposal, toilet_type, toiletshare, air_quality,
                    anganwadi_support, season)
```

global cat_child i.age_group i.female
global child age_group female
global predictors maternal_height firstbirth_18y preg_iron hhadults rural state bpl_card caste religion  siblings sibling_under5 ///
brothers_home sisters_home pre_birth_interval water_treatment toilet_type  water_home floor_finish wall_finish roof_finish house_finish use_soap ///
kitchen cooking_fuel reading
global cat_predictors i.maternal_height i.firstbirth_18y i.preg_iron i.hhadults i.rural i.state_zones i.bpl_card i.caste i.religion ///
 i.siblings i.sibling_under5 i.brothers_home i.sisters_home i.pre_birth_interval i.water_treatment i.toilet_type ///
i.water_home i.floor_finish i.wall_finish i.roof_finish i.house_finish i.use_soap i.kitchen i.cooking_fuel i.reading ///
i.states i.dist_ciaf_decile 

```{r}
# Define categories for age_group and female variables
cat_child <- c("0-5 months", "6-11 months", "12-23 months", "24-35 months", "36-47 months", "48-59 months")
cat_age_group <- c("0-5 months", "6-11 months", "12-23 months", "24-35 months", "36-47 months", "48-59 months")

# Define variables for predictors and categorical predictors
predictors <- c("maternal_height", "firstbirth_18y", "preg_iron", "hhadults", "rural", "state", "bpl_card", 
                "caste", "religion", "siblings", "sibling_under5", "brothers_home", "sisters_home", 
                "pre_birth_interval", "water_treatment", "toilet_type", "water_home", "floor_finish", 
                "wall_finish", "roof_finish", "house_finish", "use_soap", "kitchen", "cooking_fuel", 
                "reading")
cat_predictors <- c("maternal_height", "firstbirth_18y", "preg_iron", "hhadults", "rural", "state_zones", 
                    "bpl_card", "caste", "religion", "siblings", "sibling_under5", "brothers_home", 
                    "sisters_home", "pre_birth_interval", "water_treatment", "toilet_type", "water_home", 
                    "floor_finish", "wall_finish", "roof_finish", "house_finish", "use_soap", "kitchen", 
                    "cooking_fuel", "reading", "states", "dist_ciaf_decile")
```

global model_vars house_finish m_ht maternal_education toilet  cooking_fuel female christian low_caste
global cat_model_vars ib(2).house_finish ib(1).m_ht ib(3).maternal_education ///
ib(1).toilet i.state_zones  ib(1).cooking_fuel ib(1).female ib(1).christian i.low_caste

```{r}
# Define variable names
model_vars <- c("house_finish", "m_ht", "maternal_education", "toilet", "cooking_fuel", "female", "christian", "low_caste")
cat_model_vars <- c("as.factor(house_finish)", "as.factor(m_ht)", "as.factor(maternal_education)", "as.factor(toilet)", "as.factor(cooking_fuel)", "as.factor(female)", "as.factor(christian)", "as.factor(low_caste)")

# Combine the variable names into a single string
model_formula <- paste(cat_model_vars, collapse = " + ")
...
quietly logistic ciaf ib(2).house_finish ib(1).m_ht ib(3).maternal_education ///
ib(1).toilet i.state_zones  ib(1).cooking_fuel ib(1).female  i.low_caste i.priority_state, coef
quietly predict prob if e(sample)
gen missing_covariates = 1 if prob == . 
capture drop prob


```{r}
# Define category variables
cat_child <- c("age_group", "female")

# Define predictor variables
child <- c("maternal_height", "firstbirth_18y", "preg_iron", "hhadults", "rural", "state", "bpl_card", "caste", "religion",
           "siblings", "sibling_under5", "brothers_home", "sisters_home", "pre_birth_interval", "water_treatment", "toilet_type",
           "water_home", "floor_finish", "wall_finish", "roof_finish", "house_finish", "use_soap", "kitchen", "cooking_fuel",
           "reading")

# Define categorical predictor variables
cat_predictors <- c("maternal_height", "firstbirth_18y", "preg_iron", "hhadults", "rural", "state_zones", "bpl_card", "caste",
                    "religion", "siblings", "sibling_under5", "brothers_home", "sisters_home", "pre_birth_interval",
                    "water_treatment", "toilet_type", "water_home", "floor_finish", "wall_finish", "roof_finish",
                    "house_finish", "use_soap", "kitchen", "cooking_fuel", "reading", "states", "dist_ciaf_decile")

# Define model variables
model_vars <- c("house_finish", "m_ht", "maternal_education", "toilet", "cooking_fuel", "female", "christian", "low_caste")

# Define categorical model variables
cat_model_vars <- list(ib2_house_finish = "house_finish", ib1_m_ht = "m_ht", ib3_maternal_education = "maternal_education",
                       ib1_toilet = "toilet", i_state_zones = "state_zones", ib1_cooking_fuel = "cooking_fuel",
                       ib1_female = "female", i_low_caste = "low_caste")

# Select variables for logistic regression
logistic_data <- df %>%
  select(c("ciaf", model_vars, cat_model_vars, cat_child)) %>%
  drop_na() # Drop rows with missing values

# Fit logistic regression model
logistic_model <- glm(ciaf ~ ., data = logistic_data, family = binomial(link = "logit"))

# Predict probabilities
logistic_data$prob <- predict(logistic_model, type = "response")

# Create missing_covariates variable
logistic_data$missing_covariates <- ifelse(is.na(logistic_data$prob), 1, 0)

# Drop prob variable
logistic_data <- logistic_data %>% select(-prob)
```
...
df$scale_ciaf1_house <- ifelse(df$house_finish == 2, 0,
                               ifelse(df$house_finish == 1, 3,
                                      ifelse(df$house_finish == 0, 4.5, df$scale_ciaf1_house)))
```
...
df$scale_ciaf1_mheight <- ifelse(df$m_ht == 1, 0,
                                 ifelse(df$m_ht == 2, 4,
                                        ifelse(df$m_ht == 3, 8, df$scale_ciaf1_mheight)))
```
...
df$scale_ciaf1_medu <- ifelse(df$maternal_education == 3, 0,
                              ifelse(df$maternal_education == 2, 2,
                                     ifelse(df$maternal_education == 1, 3.5,
                                            ifelse(df$maternal_education == 0, 5, df$scale_ciaf1_medu))))
```
...
df$scale_ciaf1_zone <- ifelse(df$state_zones == 1, 0,
                              ifelse(df$state_zones %in% c(2, 3), 2.5,
                                     ifelse(df$state_zones %in% c(4, 6), 2,
                                            ifelse(df$state_zones == 5, 5, df$scale_ciaf1_zone))))
```
...
df$scale_ciaf1_female <- ifelse(df$female == 1, 0,
                                ifelse(df$female == 0, 1, df$scale_ciaf1_female))
```
...
# Calculate mean CIAF by shdistri and sort
df <- df %>% group_by(shdistri) %>% arrange(shdistri) %>% 
...
```{r}
# Calculate the mean 'wasted' by 'shdistri' and create a new column 'dist_wasted_mean'
df <- df %>%
  group_by(shdistri) %>%
...
*drop if child_age > 60 | stunted ==.
*count
*scalar all_under5 = r(N)
*drop if child_age > 24 
*count
*scalar all_under2 = r(N)
*drop if bidx != 1
*count
*scalar lastchild_under2 = r(N)
*drop if missing_covariates == 1 
*count
*scalar sample_cca = r(N)
capture drop _merge

/*log using "results\scratch_priority_districts.smcl"
levelsof state, local(state_num)
foreach l of local state_num {
tab sdistri state if state == `l' 
tab sdistri state if state == `l' , nolab
}
log close
translate "results\scratch_priority_districts.smcl" "results\scratch_priority_districts.pdf"
*/
...
df <- df %>% 
  group_by(shdistri) %>% 
  mutate(dist_wasted_mean = mean(wasted)) %>% 
  ungroup() %>% 
  mutate(mean_wasted_dist = case_when(
           dist_wasted_mean <= 0.399999 ~ "1. < 40%",
           dist_wasted_mean <= 0.6 & dist_wasted_mean > 0.399999 ~ "2. 40-60%",
           dist_wasted_mean > 0.6 ~ "3. > 60%"
         ),
         dist_wasted_decile = round(dist_wasted_mean, 0.1),
         wasted_decile = case_when(
           dist_wasted_decile == 0 ~ "0-10%",
           dist_wasted_decile == 0.1 ~ "10-20%",
           dist_wasted_decile == 0.2 ~ "20-30%",
           dist_wasted_decile == 0.3 ~ "30-40%",
           dist_wasted_decile == 0.4 ~ "40-50%",
           dist_wasted_decile == 0.5 ~ "50-60%",
           TRUE ~ ""
         )) 
```
