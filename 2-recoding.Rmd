...
df <- df %>%
  ...
  mutate(paternal_height = fct_case_when(
...
df <- df %>%
  mutate(still_breastfed = if_else(m4 == 95, 1, if_else(m4 != 95 & child_age <= 60, 0, NA_integer_)))
```
...
df <- df %>%
  mutate(icfi_breastmilk_score = fct_case_when(
...
df <- df %>%
  mutate(icfi_bottlefeed_score = if_else(child_age >= 6 & child_age <= 60 & m38 == 0, 1,
                                         if_else(child_age >= 6 & child_age <= 60 & m38 == 1, 0, NA_integer_)))
```
...
df <- df %>%
  mutate(icfi_starchy_staples_score = if_else(child_age >= 6 & child_age <= 24, 0,
                                              if_else((v412a == 1 | v412b == 1 | v414e == 1 | v414f == 1) & child_age >= 6 & child_age <= 60, 1, NA_integer_)))
```
...
df <- df %>%
  mutate(icfi_lentils_score = if_else(child_age >= 6 & child_age <= 60, 0,
                                      if_else(v414o == 1 & child_age >= 6 & child_age <= 60, 1, NA_integer_)))
```
...
df <- df %>%
  mutate(icfi_milk_score = if_else(child_age >= 6 & child_age <= 60, 0,
                                   if_else(v411 == 1 & child_age >= 6 & child_age <= 60, 1, NA_integer_)))
```
...
df <- df %>%
  mutate(icfi_dairy_score = if_else(child_age >= 6 & child_age <= 60, 0,
                                    if_else((v414p == 1 | v414q == 1) & child_age >= 6 & child_age <= 60, 1, NA_integer_)))
```
...
df <- df %>%
  mutate(icfi_meat_score = if_else(child_age >= 6 & child_age <= 60, 0,
                                   if_else((v414a == 1 | v414m == 1) & child_age >= 6 & child_age <= 60, 1, NA_integer_)))
```
...
df <- df %>%
  mutate(icfi_eggs_fish_score = if_else(child_age >= 6 & child_age <= 60, 0,
                                        if_else((v414g == 1 | v414n == 1) & child_age >= 6 & child_age <= 60, 1, NA_integer_))) %>%
...
df <- df %>%
  mutate(icfi_vitA_score = if_else(child_age >= 6 & child_age <= 60, 0,
                                   if_else((v414i == 1 | v414j == 1 | v414k == 1) & child_age >= 6 & child_age <= 60, 1, NA_integer_)))
```
...
df <- df %>%
  mutate(icfi_other_fruits_score = if_else(child_age >= 6 & child_age <= 60, 0,
                                           if_else(v414l == 1 & child_age >= 6 & child_age <= 60, 1, NA_integer_)))
```
...
df <- df %>%
  mutate(dietary_diversity_score = rowSums(cbind(icfi_milk_score, icfi_meat_score, icfi_lentils_score, icfi_dairy_score,
         ...
         dietary_diversity = case_when(
           ...
           is.na(dietary_diversity_score) ~ NA_integer_
         ))
...
df <- df %>% 
  mutate(toilet_type = fct_case_when(
...
df <- df %>% 
  mutate(last_child_id = ifelse(midx == 1, real_id, NA_integer_)) 
