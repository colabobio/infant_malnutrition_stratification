...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
SOME LOGISTIC REGRESSION CODE, NOT CLEAR WHAT FOR
...
```{r}
df0 <- data.frame(df, copy = TRUE)
```

```{r}
df <- data.frame(df0, copy = TRUE)
```

```{r}
# Removed missing variables: bpl_card

# Reorder variables in the data frame
df <- df %>% arrange(wgt, real_id, mother_id, father_id, bidx, haz, waz, whz, stunted, wasted, underweight, ciaf,
                    ...
                    wealth_quintile, hhadults, rural, state, caste, religion,
                    total_living_children, siblings, sibling_under5, brothers_home, sisters_home,
                    pre_birth_interval, succeeding_birth_interval, multiplet, dietary_diversity_score, bf_start, vitamin_a,
...
assign("cat_child", cat_child, envir = .GlobalEnv)

child <- c("age_group", "female")
assign("child", child, envir = .GlobalEnv)

# "bpl_card" variable removed as it absent in the new dataset

predictors <- c("maternal_height", "firstbirth_18y", "preg_iron", "hhadults", "rural", "state", "caste", "religion", "siblings", 
                "sibling_under5", "brothers_home", "sisters_home", "pre_birth_interval", "water_treatment", "toilet_type", 
                "water_home", "floor_finish", "wall_finish", "roof_finish", "house_finish", "use_soap", "kitchen", "cooking_fuel", "reading")
assign("predictors", predictors, envir = .GlobalEnv)

cat_predictors <- c("maternal_height", "firstbirth_18y", "preg_iron", "hhadults", "rural", "state_zones",
                    "caste", "religion", "siblings", "sibling_under5", "brothers_home",
...
assign("cat_predictors", cat_predictors, envir = .GlobalEnv)
#df[, cat_predictors] <- lapply(df[, cat_predictors], factor)
...
ind_vars <- model.matrix(~ color - 1, data = df)

df <- cbind(df, ind_vars)
...
child <- c("maternal_height", "firstbirth_18y", "preg_iron", "hhadults", "rural", "state", "caste", "religion",
...
cat_predictors <- c("maternal_height", "firstbirth_18y", "preg_iron", "hhadults", "rural", "state_zones", "caste",
