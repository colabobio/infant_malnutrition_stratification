...
```{r}
const_vars <- c()
for (var in colnames(dfc)) {
  unique_values <- unique(dfc[[var]])
...
missing_na_df_precentage = missing_na_df %>% dplyr::mutate(`Percentage of NAs` = as.integer(`Number of NAs` / as.numeric(nrow(dfc)) * 100)) %>% arrange(desc(`Number of NAs`))
  missing_na_df_precentage
```
```{r}
path <- "data/"
nafile <- "percentageNA.csv"
...
```{r}
dfc <- dfc %>% select(-all_of(HighPrecentageofNAs))
dfc
```
...
path <- "data/"
dffile <- "cleaned.rda"
save(dfc, file = here(path, dffile))
```
